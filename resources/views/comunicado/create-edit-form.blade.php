        ...
        <form action="{{ $comunicado ? route('comunicados.edit', ['id' => $comunicado->id]) : route('comunicados.create') }}" method="post">
            ...
            <div class="mb-4">
                <div class="inline-block relative w-full">
                    <select name="user_id" for="user_id" class="@error('user_id') border-red-500 @enderror block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">

                        <option value="" disabled>Elija el usuario destinatario</option>
                        @foreach($usuarios as $usuario)
                            @if($comunicado)
                            <option value="{{ $usuario->id }}"  {{ $usuario->id === $comunicado->user_id ? 'selected' : ''}}>{{ $usuario->name }}</option>
                            @else
